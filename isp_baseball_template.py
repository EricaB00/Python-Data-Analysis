...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    for stat in statistics:
        top_player_id.append((stat[info['playerid']], formula(info,stat)))
    top_player_id = sorted(top_player_id, key=lambda x: x[1], reverse=True)
    return top_player_id[0:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    stat_file = read_csv_as_list_dict(info['masterfile'], info['separator'], info['quote'])

    name_table = []
    for ids in top_ids_and_stats:
        for stat in stat_file:
            if stat[info['playerid']] == ids[0]:
                name_table.append('{0:.3f} --- {1} {2}'.format(ids[1],
                    stat[info['firstname']], stat[info['lastname']]))
    return name_table
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    bt_statfile = read_csv_as_list_dict(info['battingfile'], info['separator'], info['quote'])

    sp_year = filter_by_year(bt_statfile, year, info['yearid'])
    top_player = top_player_ids(info, sp_year, formula, numplayers)
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for data in statistics:
        dict_stats[data[playerid]] = {}
        for field in fields:
            dict_stats[data[playerid]][field] = 0
            dict_stats[data[playerid]][playerid] = data[playerid]

    for key in dict_stats:
        for data in statistics:
            if key == data[playerid]:
                for stat in fields:
                    dict_stats[key][stat] += int(data[stat])
    return dict_stats
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    solo_player_stat = aggregate_by_player_id(bat_stat, info['playerid'], fieldnames)
    player_name = [k for k,v in solo_player_stat.items()]

    full_stat = []
    for player in player_name:
        full_stat.append((player, formula(info, solo_player_stat[player])))
    full_stat = sorted(full_stat, key=lambda x:x[1], reverse=True)
    return lookup_player_names(info, full_stat[0:numplayers])
